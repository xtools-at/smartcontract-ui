											...
											for (let key in args) {
												...
												output.push(
													<ListItemButton
														...
														key={key}
														onClick={() => copyText(args[key])}
													>
														<ListItemText
															primary={String(args[key])}
															secondary={key}
														/>
													</ListItemButton>
												);
							...
							return (
								<>
									...
									>
										<li>
											<ul style={{ margin: 0, padding: 0 }}>
													...
													<ListItemButton
														...
														onClick={() => {
															...
															if (
																...
																window.open(
																	`${chain.explorers[0].url}/block/${result.data.waitResult.blockNumber}`
																);
															}
														}}
													>
														<ListItemText
															primary={`${result.data.waitResult.blockNumber} (${result.data.waitResult.blockHash})`}
															secondary="Block"
														/>
													</ListItemButton>
												)}
												<ListItemButton
													...
													onClick={() => {
														...
														if (chain.explorers && chain.explorers.length > 0) {
															window.open(
																`${chain.explorers[0].url}/tx/${result.data.hash}`
															);
														}
													}}
												>
													<ListItemText
														primary={result.data.hash}
														secondary="TX Hash"
													/>
												</ListItemButton>
												<ListItemButton
												...
												>
													<ListItemText
														primary={`${(() => {
															const decimal = BigNumber.from(10).pow(
														...
														} (${result.data.value.toString()} wei)`}
														secondary={`Value`}
													/>
												</ListItemButton>
											</ul>
										...
										<li>
											{result.data.waitResult &&
												result.data.waitResult.events.length > 0 && (
													<ul style={{ margin: 0, padding: 0 }}>
														<ListSubheader
															sx={{
																backgroundColor: "divider",
														...
														{result.data.waitResult.events.map(
															(event: WriteEvent, index: number) => (
																<ul
