...
export const ContractSelector = ({
	...
	const [faucetAnchorEl, setFaucetAnchorEl] = useState<null | HTMLElement>(
		null
	);
	...
	return (
		<>
			<Grid container spacing={1} justifyContent="space-between">
				<Grid item xs={hasFaucet ? 9 : 12} md={hasFaucet ? 10 : 12}>
					<Autocomplete
						disablePortal
						id="chains"
						fullWidth
						...
						renderInput={(params) => (
							<TextField {...params} required label="Network" />
						)}
						value={chain}
				...
				{hasFaucet && (
					<>
						...
						>
							<Tooltip
								arrow
								title={`Get free ${chain.nativeCurrency.name} (${chain.nativeCurrency.symbol})`}
							>
								<Button
									variant="outlined"
									onClick={(evt) => {
