...
export class NoBscProviderError extends Error {
	public constructor() {
		...
		this.message =
			"No BSC provider was found on (global as any)['BinanceChain'].";
	}
}
...
export class BscConnector extends AbstractConnector {
	...
	private handleNetworkChanged(networkId: string | number): void {
		...
		this.emitUpdate({
			chainId: networkId,
			provider: (global as any)["BinanceChain"],
		});
	}
	...
	public async activate(): Promise<ConnectorUpdate> {
		...
		if (((global as any)["BinanceChain"] as any).isMetaMask) {
			((global as any)["BinanceChain"] as any).autoRefreshOnNetworkChange =
				false;
		}
		...
		if (!account) {
			...
			account = await (global as any)["BinanceChain"]
				.enable()
				.then(
	...
	public async getChainId(): Promise<number | string> {
		...
		let chainId;
		try {
			chainId = await ((global as any)["BinanceChain"].send as Send)(
				"eth_chainId"
		...
		if (!chainId) {
			try {
				chainId = parseSendReturn(
					((global as any)["BinanceChain"].send as SendOld)({
						method: "net_version",
					})
				);
			} catch {
	...
	public async isAuthorized(): Promise<boolean> {
		...
		try {
			return await ((global as any)["BinanceChain"].send as Send)(
				"eth_accounts"
			).then((sendReturn) => {
